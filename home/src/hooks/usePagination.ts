...
interface Options {
  pageSize?: number;
  /** When this value changes, jump back to page 1 (e.g. the chapter id). */
  resetKey?: unknown;
}
...
export function usePagination<T>(
  ...
  // Back to page 1 when the source list changes (new chapter, new filter…).
  useEffect(() => {
    setPage(1);
  }, [resetKey]);

  // Never leave the current page beyond the last one (after reloads, deletes…).
  useEffect(() => {
